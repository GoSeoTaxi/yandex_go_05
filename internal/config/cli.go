...
func InitCLI() {
	...
	if len(DBStringConnect) > 1 && err == nil && ping == nil {
		_, err := db.Exec("create table if not exists shortyp10 (id    integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,link  varchar(256) NOT NULL,login varchar(256));")
		if err != nil {
			fmt.Println(err)
		}
